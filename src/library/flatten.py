...
def process_hash_list(document_datasets):
    ...
    for file_data in document_datasets:
        try:
            ...
            logger.info('Flattening file with hash {} doc id {}, downloaded at {}'.format(file_hash, doc_id, downloaded.isoformat()))
...
def main():
    ...
    if config['FLATTEN']['PARALLEL_PROCESSES'] == 1:
        ...
        logger.info("Flattening and storing " + str(len(file_hashes)) + " IATI files in a maximum of " + str(config['FLATTEN']['PARALLEL_PROCESSES']) + " parallel processes.")
