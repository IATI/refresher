...
def addToSolr(conn, core_name, batch, file_hash):
    ...
    try:
        ...
        if status_code >= 500:
            logger.warning('Solr reports Client Error ' + str(status_code) + ' for source blob ' + file_hash + '.xml. Sleeping for ' + str(config['SOLRIZE']['SOLR_500_SLEEP']) + ' seconds: ' + e.args[0])
