...
def copy_valid():
    ...
    if config['CLEAN']['PARALLEL_PROCESSES'] == 1:
        logger.info(
            f"Copying {len(documents)} valid IATI files in a single process.")
        copy_valid_documents(documents)
    else:
        chunked_doc_lists = list(utils.chunk_list(
            documents, config['CLEAN']['PARALLEL_PROCESSES']))
...
def clean_invalid_documents(documents):
    ...
    try:
        ...
        for document in documents:
            ...
            if len(cleanDoc) == 0:
                logger.info(
                    f"No valid activities for hash: {hash} id: {id}. ")
            ...
            blob_client.set_blob_tags(
                {"dataset_hash": hash, "document_id": id})
...
def clean_invalid():
    ...
    """
    logger.info(
        "Starting clean of invalid activities documents to save to clean container...")
    ...
    if config['CLEAN']['PARALLEL_PROCESSES'] == 1:
        logger.info(
            f"Cleaning and storing {len(documents)} IATI files in a single process.")
