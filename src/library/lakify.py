...
def recursive_json_nest(element, output):
    ...
    if element.text is not None and element.text.strip() != "":
        ...
        if element.tag is not etree.Comment and element.tag is not etree.PI:
            inner_text = "".join([inner_string.strip() for inner_string in element.itertext(tag=element_tag)])
    ...
    for file_data in document_datasets:
        try:
            ...
            if root.tag != "iati-activities":
                raise Exception(
                    'Blob returning non-IATI XML for doc id {} with hash {}. Returned: "{}"'.format(
            ...
            if hasattr(e, "msg"):
                err_message = e.msg
            logger.error(
                "ERROR with Lakifiying hash {} and doc id {}. Error: {}".format(file_hash, doc_id, err_message)
            )
...
def main():
    ...
    if config["LAKIFY"]["PARALLEL_PROCESSES"] == 1:
        ...
        chunked_hash_lists = list(utils.chunk_list(file_hashes, config["LAKIFY"]["PARALLEL_PROCESSES"]))
        ...
        logger.info(
            ...
            + str(config["LAKIFY"]["PARALLEL_PROCESSES"])
            + " parallel processes"
        )
